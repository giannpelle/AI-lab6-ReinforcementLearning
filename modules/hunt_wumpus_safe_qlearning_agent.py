...
from modules.hunt_wumpus_state_safe import HuntWumpusStateSafe

class HuntWumpusSafeQLearningAgent(object):
    """
    defines a Q-Learning agent able to solve an Hunt the Wumpus game
    where there is a clear and safe path to reach the gold
    (and no wumpus is blocking the way)
    in a memory efficient way (small qtable size)
    """

    def __init__(self, env):
        self.env = env
        self.q_table = np.zeros([HuntWumpusStateSafe.get_state_space_size(), env.action_space.n])
    ...
    def train(self, *, episodes: int, alpha=0.2, gamma=0.7) -> Tuple[np.ndarray, Sequence[int]]:
        ...
        for episode in range(episodes):
            ...
            obs = self.env.reset()
            hunt_wumpus_state = HuntWumpusStateSafe()
